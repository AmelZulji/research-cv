...
local function preamble(out, cv, theme)
  ...
  emit(out, "  }]],")
  emit(out, ")")
  emit(out, "#let page_footer(label) = place(top + left, dx: 0mm, dy: page_h - footer_bottom_margin - footer_size)[")
  emit(out, "  #box(width: page_w - footer_right_margin)[#align(right)[#text(size: footer_size, fill: muted, label)]]")
  emit(out, "]")
end
...
local function render_first_page(out, cv)
  ...
  emit(out, "      #set par(justify: true, leading: profile_leading)")
...
local function render_second_page(out, cv)
  emit(out, "#page()[")
  emit(out, "  #page_footer(" .. q(value_text((cv.person or {}).name) .. " · 2 / 2") .. ")")
